...
def run_throughput(host, collection_name, num_documents, log_file, **kwargs):
    """Runs a throughput benchmark against a ChromaDB service."""
    print(f"Starting ChromaDB throughput benchmark on {host}...")
    print(f"Collection: '{collection_name}', Documents: {num_documents}")
    ...
    try:
        ...
        # --- Write throughput ---
        print(f"Writing {num_documents} documents...")
        start_time = time.time()
        collection.add(
            ids=[f"id_{i}" for i in range(int(num_documents))],
            documents=[f"This is document {i}" for i in range(int(num_documents))]
        )
        end_time = time.time()
        write_duration = end_time - start_time
        write_throughput = int(num_documents) / write_duration if write_duration > 0 else float('inf')

        write_summary = {
            "operation": "write",
            "documents": int(num_documents),
        ...
        # --- Read throughput ---
        print(f"\nReading {num_documents} documents...")
        start_time = time.time()
        collection.get(ids=[f"id_{i}" for i in range(int(num_documents))])
        end_time = time.time()
        read_duration = end_time - start_time
        read_throughput = int(num_documents) / read_duration if read_duration > 0 else float('inf')
...
def run_query(host, collection_name, num_documents, n_results, log_file, **kwargs):
    """Runs a query performance benchmark against a ChromaDB service."""
    print(f"Starting ChromaDB query benchmark on {host}...")
    print(f"Collection: '{collection_name}', Documents: {num_documents}, N_results: {n_results}")
    ...
    try:
        ...
        collection.add(
            embeddings=[[i/10.0, i/10.0, i/10.0] for i in range(int(num_documents))],
            ids=[f"id_{i}" for i in range(int(num_documents))]
        )
        ...
        summary = {
            "operation": "query",
            "documents_in_collection": int(num_documents),
            "n_results": int(n_results),
